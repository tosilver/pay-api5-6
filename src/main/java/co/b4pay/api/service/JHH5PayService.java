...
@Service
//@Transactional
public class JHH5PayService extends BasePayService {

    private static final Logger logger = LoggerFactory.getLogger(JHH5PayService.class);
    ...
    public JSONObject executeReturn(Long merchantId, Router router, JSONObject params, HttpServletRequest request) throws BizException {
        ...
        try {
            ...
            String result = HttpsUtils.post(channel.getIp4() + "/pay/JHH5Execute.do", null, m);
    ...
    public JSONObject execute(Long merchantId, Router router, JSONObject params, HttpServletRequest request) throws Exception {
        ...
        paramMap.put("agre_type", "T");
        paramMap.put("inst_no", "10000094");
        paramMap.put("merch_id", "100000941000167");
        paramMap.put("pay_type", "040");
        paramMap.put("commodity_name", "Test");
        paramMap.put("amount", totalAmount.toPlainString());
        paramMap.put("back_end_url", String.format("%s/notify/JHH5PayNotify.do", serverUrl));
        paramMap.put("return_url", notifyUrl);
        paramMap.put("merch_order_no",tradeId);
        ...
        System.out.println("[聚合H5支付]应答报文:  " + responseCreateOrder);
        if (responseCreateOrder != null) {
            ...
            if (StringUtils.isNotBlank(rspJson.getString("retcode")) && "00".equals(rspJson.getString("retcode"))) {
                String qrcode = rspJson.getString("payInfo");
                String platformOrderNo = rspJson.getString("platform_order_no");
                String retmsg = rspJson.getString("retmsg");
                logger.info("[JHH5]支付链接:",qrcode);
                logger.info("[JHH5]支付响应msg",retmsg);
                ...
                trade.setId(tradeId);
                trade.setAddressId(Long.parseLong("001"));
                ...
                trade.setStatus(1);
                trade.setPayOrderNo(platformOrderNo);
                tradeDao.save(trade);
                logger.warn("JHH5 trade ->" + JSONObject.toJSONString(trade));
                ...
                logger.warn("[JHH5下单]交易失败:" + msg);
                logger.warn("渠道调用异常，关闭账户！！！！");
    ...
    public JSONObject ordernquiry(JSONObject params) throws IOException, WriterException {
        ...
        HashMap<String, String> resultInfo = getResultInfo(result, jsonString, secretKey, kJPAY_API_DOMAIN);
        String responseCreateOrder = resultInfo.get("responseCreateOrder");
        System.out.println("[聚合H5支付]查询应答报文:  " + responseCreateOrder);
        JSONObject rspJson = JSONObject.parseObject(responseCreateOrder);
        if (responseCreateOrder != null) {
            if (StringUtils.isNotBlank(rspJson.getString("retcode")) && "00".equals(rspJson.getString("retcode"))) {
                String qrcode = rspJson.getString("payInfo").replace("\\", "");
                logger.info("[JHH5支付]支付链接:", qrcode.toString());
                JSONObject jsonObject = new JSONObject();
                jsonObject.put("out_trade_no",outTradeNo);
                jsonObject.put("msg", "接口调用成功");
                jsonObject.put("code", "10000");
