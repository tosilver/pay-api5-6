...
@Service
//@Transactional
public class MallPayTestService extends BasePayService {

    private static final Logger logger = LoggerFactory.getLogger(MallPayTestService.class);
    //回调地址
    ...
    public JSONObject executeReturn(Long merchantId, Router router, JSONObject params, HttpServletRequest request) throws BizException {
        ...
        logger.info("[新通道]MallPayService-->executeReturn:" + params);
        ...
        logger.warn("[新通道]支付金额为:" + totalMOney);
        Channel channel = getChannel(merchantId, router, totalMOney);// 预校验
        logger.warn("[新通道]通道:" + channel.getName());
        ...
        try {
            ...
            String result = HttpsUtils.post(channel.getIp4() + "/pay/mallPayTestExecute.do", null, m);
    ...
    public JSONObject execute(Long merchantId, Router router, JSONObject params, HttpServletRequest request) throws Exception {
        ...
        jsonObject.put("RequestedURL", sb.toString());
        //给查询端口发送请求参数
        GetToQuery.getToQuery2(tradeId,i,pay_type,address);
        mallCheckOutService.timer1(outTradeNo);
        logger.info("[新通道]响应的参数为:"+jsonObject.toString());
        return jsonObject;
    }
