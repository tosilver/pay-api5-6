 ...
 */
@RestController
public class MallSPayTestController extends BaseController {
    private static final Logger logger = LoggerFactory.getLogger(MallSPayTestController.class);
    ...
    @Autowired
    private MallPayTestService mallPayTestService;


    @RequestMapping(value = "/pay/mallTestPay.do", method = RequestMethod.POST)
    public Object aliSPay(HttpServletRequest request) {
        try {
            Router router = routerService.findById(ROUTER_KEY);
            return mallPayTestService.executeReturn(getMerchantId(request), router, getParams(request), request);
    ...
    @RequestMapping(value = "/pay/mallPayTestExecute.do", method = RequestMethod.POST)
    public AjaxResponse aliSPayExecute(HttpServletRequest request) {
        try {
            Router router = routerService.findById(ROUTER_KEY);
            JSONObject jsonObject = mallPayTestService.execute(getMerchantId(request), router, getParams(request), request);
