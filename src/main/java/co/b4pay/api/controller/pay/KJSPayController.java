 ...
 */
@RestController
public class KJSPayController extends BaseController {
    private static final Logger logger = LoggerFactory.getLogger(KJSPayController.class);

    private static final String ROUTER_KEY = "kjPay";

    private static final String[] REQUIRED_PARAMS =
            new String[]{"tradeNo", "subject", "totalAmount", "notifyUrl","commodity_name","money","ip","deviceType","returnUrl","agre_type","pay_type"};
    ...
    @Autowired
    private KJPayService kjPayService;

    @RequestMapping(value = "/pay/kjPay.do", method = RequestMethod.POST)
    public Object aliSPay(HttpServletRequest request) {
        try {
            Router router = routerService.findById(ROUTER_KEY);
            return kjPayService.executeReturn(getMerchantId(request), router, getParams(request), request);
    ...
    @RequestMapping(value = "/pay/kjPayExecute.do", method = RequestMethod.POST)
    public AjaxResponse aliSPayExecute(HttpServletRequest request) {
        try {
            Router router = routerService.findById(ROUTER_KEY);
            JSONObject jsonObject = kjPayService.execute(getMerchantId(request), router, getParams(request), request);
