 ...
 */
@RestController
@RequestMapping({"/pay/personalCode.do", "/pay/fixedCode.do"})
public class PersonalFixedCodePayController extends BaseController {

    private static final Logger logger = LoggerFactory.getLogger(PersonalFixedCodePayController.class);
    private static final String ROUTER_KEY = "fixedCodePay";
    ...
    public AjaxResponse doPost(HttpServletRequest request, HttpServletResponse response) {
        try {
            ...
            if (router == null || router.getStatus() == -1) {
                throw new RuntimeException(String.format("[%s]路由异常", "fixedCodePay"));
            }
            return codePayService.executeWithFixed(getMerchantId(request), getParams(request), request, router);
